...
app.controller('TranslationController', [
  ...
  function ($scope, $http, $timeout, $location, $anchorScroll, ngTableParams, sharedMessage) {
    ...
    $scope.columns = [
      ...
      { title: translationCfg.label.key, index: '_key', edit: false, filter: {'_key': 'text'}, sortable: true, visible: false },
      { title: 'Statut', index: '_status', edit: false, filter: true, sortable: true, visible: true }
      ...
      getData: function($defer, params) {
        ...
        if (Object.keys(params.filter()).length) {
          ...
          for (var key in params.filter()) {
            ...
            if (key == '_status'){
              $scope.currentStatus = parameters[key];
            }
          }
  ...
  return {
    ...
    link: function ( $scope, element, attrs ) {
      ...
      $scope.validate = function (event) {
        var url = translationCfg.url.validate.replace('-id-', $scope.translation._id);

        $http({ 'url': url, method: 'GET' })
          .success(function (data, status, headers, config) {
            $scope.translation._status = 3;
            sharedMessage.set('success', 'ok-circle', 'Traduction validée.');
          }).error(function (data, status, headers, config) {
            sharedMessage.set('danger', 'remove-circle', 'Erreur lors de la validation de la traduction.');
          });
      };

      $scope.invalidate = function (event) {
        var url = translationCfg.url.invalidate.replace('-id-', $scope.translation._id);

        $http({ 'url': url, method: 'GET' })
          .success(function (data, status, headers, config) {
            $scope.translation._status = 1;
            sharedMessage.set('success', 'ok-circle', 'Traduction révoquée.');
          }).error(function (data, status, headers, config) {
            sharedMessage.set('danger', 'remove-circle', 'Erreur lors de la révocation de la traduction.');
          });
      };

      $scope.waiting = function (event) {
        var url = translationCfg.url.waiting.replace('-id-', $scope.translation._id);

        $http({ 'url': url, method: 'GET' })
          .success(function (data, status, headers, config) {
            $scope.translation._status = 2;
            sharedMessage.set('success', 'ok-circle', 'Traduction remise à "en cours"');
          }).error(function (data, status, headers, config) {
            sharedMessage.set('danger', 'remove-circle', 'Erreur lors du changement de statut de la traduction.');
          });
      };
    }
  };
